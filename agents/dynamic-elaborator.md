...
- **Tasks run in isolated worktrees and merge at integration.** Each task implements on its own branch in its own git worktree; an integrator merges the wave back in dependency order. There are no file locks — concurrency is bounded only by `deps`. Design task scopes to keep file footprints disjoint anyway: overlapping edits don't corrupt anything, but they become merge conflicts the integrator must resolve, and heavy overlap between two tasks is a sign they should be one task or a dependency edge. `files` documents each task's expected footprint for reviewers and the integrator — it is informational, not a lock.
...
Per task: `id`, `title`, `difficulty`, `risk`, `files` (created/modified — the task's expected footprint; informational, not a lock), `deps` (task ids), `ears` (requirement ids + short text), `refs` (path — reason), `context` (≤150 words: current-state facts, decisions made, interface contracts — may include a ≤10-line signature block when tasks must agree on a shape), `tests` (one line each), `verify` (commands or criteria checkable at the phase boundary, beyond the standard suite).
